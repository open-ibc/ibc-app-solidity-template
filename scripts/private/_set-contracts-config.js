...
// Example:
// $ node set-contracts-config.js optimism XGif true
const fs = require("fs");
const { getConfigPath } = require("./_helpers.js");

if (process.argv.length < 5) {
  console.error(
    "❌ Incorrect number of args. Usage: node set-contracts-config.js <chain> <contractType> <isUniversal>"
  );
  process.exit(1);
}
...
if (chain !== "optimism" && chain !== "base") {
  console.error(
    "❌ Incorrect chain value. Usage: node set-contracts-config.js <chain> <contractType> <isUniversal>"
  );
  process.exit(1);
}
...
if (universalBoolean === "true") {
  ...
  console.error(
    "❌ Incorrect boolean value. Usage: node set-contracts-config.js <chain> <contractType> <isUniversal>"
  );
  process.exit(1);
}
...
function updateConfig(network, contractType) {
  try {
    ...
    console.log(
      `🆗 Updated config with ${contractType} for ${network}. Set isUniversal to ${isUniversal}.`
    );
  } catch (error) {
